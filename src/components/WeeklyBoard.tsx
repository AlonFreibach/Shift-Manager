...
import {
  MIYA_NAME, WEEK_STRUCTURE, SLOT_DEFAULTS, MIYA_SCHEDULE,
  isBirthdayOnDate, isOnVacation, getBaseStations, getStations, getStationBadge,
  getWeekStart, formatDate, isEmployeeAvailable,
  calculateCompositeScore, migrateScheduleIds,
} from './WeeklyBoard.utils';
import type {
  Slot, Schedule, VoltFlags, CustomShiftDef, SpecialShiftEntry,
  PlanAheadStep, PlanAheadSummary,
  ShortageItem, TieItem, TraineeResult, AutoResultModal,
  BlockConstraint, LimitConstraint, FixConstraint, HoursConstraint,
  MinConstraint, StationHoursConstraint, CloseConstraint, SchedulingConstraint,
} from './WeeklyBoard.types';
...
export function WeeklyBoard({ employees, refreshEmployees, autoScheduleRequest, onAutoScheduleHandled, onNavigateToPreferences }: WeeklyBoardProps) {
  ...
  function undo() {
    ...
    saveScheduleToStorage(weekKeyRef.current, snapshot.schedule);
  ...
  async function runPlanAheadAutoSchedule() {
    ...
    for (const sunday of sundays) {
      ...
      saveScheduleToStorage(wk, finalSchedule);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const remote = await loadScheduleFromStorage(weekKey);
      if (loadCancelled) return;
      let loadedSchedule: Schedule = migrateScheduleIds(remote, employees);
      ...
      if (changed) {
        saveScheduleToStorage(weekKey, loadedSchedule);
      }
      setSchedule(loadedSchedule);
    })();

    // Realtime: pull changes from other devices.
    const unsubscribeSchedule = subscribeToSchedule(weekKey, remote => {
      if (loadCancelled) return;
  ...
  function saveSchedule(newSchedule: Schedule, skipUndo = false) {
    ...
    saveScheduleToStorage(weekKey, newSchedule);
  }
  ...
  function generatePDF(selectedWeekKeys: string[]) {
    printSchedule(selectedWeekKeys, employees);
  }
