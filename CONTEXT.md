...
### סבב משוב שישי — שורת מיה כ"הגשה וירטואלית" בלוח התכנון
- **בעיה:** מיה היא admin ואין לה רשומות ב-`preferences` → התאים שלה בטבלת התכנון הופיעו ריקים בשבועות שעדיין לא הוגדר להם schedule ב-WeeklyBoard.
- **תיקון (PreferencesTableView.tsx):**
  - בלולאת התאים: `submitted = !!row.group?.shifts[key] || (isAdmin && t === 'morning')` — 6 משמרות הבוקר של מיה תמיד מוצגות כמוגשות.
  - עמודת "הגישה" עבור שורת admin משדרת `6` במקום `—`, בעקבות ✓.
  - שאר הלוגיקה (cell-assigned/cell-locked) נשארת — אם ה-schedule הופעל כבר, התאים יציגו צהוב + "קבועה" (כי הסלוטים של מיה ב-`schedules` שמורים עם `locked: true`); אם לא, ✓ לבן ("ביקשה, לא שובצה").
- **למה זה הגיוני:** מיה לא מגישה — מבחינת המנהלות שמסתכלות בטבלה, מצופה לראות את לוח הקבע שלה כמו של כל עובדת אחרת, גם בשבועות עתידיים שעדיין לא נפתחו ב-WeeklyBoard.
